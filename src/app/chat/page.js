...
//             // Status list
//             <div>
//               <p className="p-4 text-gray-800 font-bold bg-gray-50">
//                 Recent updates
//               </p>
...
//                       msg.sender === "You"
//                         ? "bg-green-500 text-grey-900"
//                         : "bg-white  text-grey-900 border border-gray-200" 
//                     }`}
//                   >
//                     {msg.type === "image" && (
//                       <img
...
//                       msg.type === "text" && (
//                      <p className={`break-words ${msg.sender === "You" ? "text-white" : "text-gray-900"}`}>
//   {msg.text}
//   {msg.edited && (
//     <span className={`text-xs ml-1 ${msg.sender === "You" ? "text-white/70" : "text-gray-500"}`}>
//       edited
//     </span>
//   )}
// </p>
...
//                        <p className={`break-words pt-2 ${msg.sender === "You" ? "text-white" : "text-gray-900"}`}>
//   {msg.text}
//   {msg.edited && (
//     <span className={`text-xs ml-1 ${msg.sender === "You" ? "text-white/70" : "text-gray-500"}`}>
//       edited
...
//   className="flex-1 border-2 border-green-300 rounded-full px-4 py-2 text-base text-gray-900 placeholder:text-gray-500 bg-white focus:outline-none focus:border-green-500"
// />
//               </div>
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    socket.current.on("incomingCall", ({ from, type, offer }) => {
      ...
      const playPromise = ringtone.current?.play();
      if (playPromise !== undefined) {
        playPromise.catch(() => console.log("Ringtone autoplay prevented."));
      }
    });
    ...
    socket.current.on("callRejected", ({ from }) => {
      alert(`${from} rejected the call.`);
      if (call) logCall(call.type, "dialed", me.username, call.to);
      resetCallState();
    });
  ...
  const acceptCall = async () => {
    try {
      ...
      setCallAccepted(true);
      setCall({ from: incomingCall.from, type: incomingCall.type });
      setIncomingCall(null);
      ringtone.current?.pause();
    } catch (err) {
      console.error("Error accepting call:", err);
  ...
  const rejectCall = () => {
    socket.current.emit("rejectCall", { to: incomingCall.from });
    logCall(incomingCall.type, "missed", incomingCall.from, me.username);
    resetCallState();
  };
  ...
  const postStatus = async () => {
    ...
    if (statusFile) {
      ...
      mediaUrl = data.filePath;
      mediaType = statusFile.type.startsWith("video") ? "video" : "image";
    }

    await axios.post(
      `${API_URL}/api/status`,
      { text: statusText, mediaUrl, mediaType },
      { headers: { Authorization: `Bearer ${token}` } }
    );
  ...
  const filteredUsers = users.filter((user) => {
    ...
    return searchWords.every((word) =>
      usernameWords.some((nameWord) => nameWord.includes(word))
    );
  });
            ...
            <div>
              <div className="p-3 bg-white border-b sticky top-0 z-10">
                <input
                  ...
                  className="w-full border-2 border-green-300 rounded-full px-4 py-2 text-gray-900 placeholder:text-gray-500 bg-white focus:outline-none focus:border-green-500"
                />
              </div>
...
<p className="font-semibold text-lg text-gray-800">
  ...
  >
                  ...
                  <div>
                    <p className="font-semibold text-lg">{userStatus.username}</p>
                    <p className="text-sm text-gray-500">
                      {userStatus.statuses.length} updates
                    </p>
                  </div>
          ...
          {activeTab === "calls" && (
            <div>
              {callLogs.map((log, index) => {
                ...
                return (
                  <div
                    key={log._id || `${log.caller}-${log.receiver}-${log.createdAt}-${index}`}
                    className="p-4 hover:bg-green-50 cursor-pointer border-b flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
                      <img
                    ...
                    <div className={isMissed ? "text-red-600" : "text-green-600"}>
                      {log.type === "video" ? <Video /> : <Phone />}
                    </div>
                  </div>
                );
      ...
      <div
        className={`${receiver ? "flex" : "hidden md:flex"} flex-1 flex flex-col`}
      >
        {receiver ? (
          <>
            <div className="bg-green-600 text-white p-4 flex items-center justify-between shadow-lg">
              <div className="flex items-center gap-3">
                ...
                <img
                  src={`${API_URL}${receiver.profileImage || "/default-avatar.png"}`}
                  className="w-11 h-11 rounded-full"
                  alt="receiver"
                />
                <h2 className="text-xl font-bold truncate">{receiver.username}</h2>
              </div>
  ...
  const messageKey =
    msg._id ||
    `${msg.sender}-${msg.receiver || "unknown"}-${msg.createdAt || index}-${index}`;
