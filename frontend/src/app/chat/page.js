...
// // This is necessary for WebRTC to connect peers
// const iceServers = {
//   iceServers: [
...
// // Updated StatusViewer to play video
...
//         {/* UPDATED: Conditional video/image rendering */}
...
//             autoPlay
//             loop={false}
//             onEnded={onNext} // Go to next status when video finishes
//           />
...
//   const [incomingCall, setIncomingCall] = useState(null); // Now holds { from, type, offer }
//   const [callAccepted, setCallAccepted] = useState(false);
//   const [callLogs, setCallLogs] = useState([]); // For call history
...
//         setCallLogs(callsRes.data); // <-- Set call logs
//       } catch (err) {
...
//       const formatted = {
//         ...msg,
//         sender: msg.sender === me.username ? "You" : msg.sender,
//         reactions: msg.reactions || {},
//       };
//       if (
//         receiver &&
//         (msg.sender === receiver.username || msg.receiver === me.username)
//       ) {
//         setMessages((prev) => [...prev, formatted]);
//       } else if (msg.receiver === me.username) {
//         setUnread((prev) => ({
//           ...prev,
...
//     // --- 📞 UPDATED CALL LISTENERS ---

//     // 1. Incoming Call (now with offer)
//     socket.current.on("incomingCall", ({ from, type, offer }) => {
//       setIncomingCall({ from, type, offer }); // Store the offer
...
//     // 2. Call Accepted (now with answer)
...
//       logCall(call.type, "dialed", me.username, call.to); // Log as dialed
//       resetCallState();
//     });
...
//     };
//   }, [me, receiver, call]); // Add 'call' dependency

//   /* ------------------------------------------------------------------ */
//   /*  WebRTC Core Functions                                            */
//   /* ------------------------------------------------------------------ */

//   // Gets user's camera/mic
...
//   stream.getTracks().forEach(track => { // <-- 2. Use 'stream' argument
//     peerConnection.addTrack(track, stream); // <-- 3. Use 'stream' argument
//   });
...
//     // --- (NEW) Get username from localStorage ---
...
//       data.map((m) => ({
//         ...m,
//         // --- (UPDATED) Use myUsername for the check ---
...
//       { username: newName, profileImage: picUrl },
//       { headers: { Authorization: `Bearer ${token}` } }
//     );
...
//   /* ------------------------------------------------------------------ */
//   /*  STATUS VIEWER LOGIC (Auto-advance for image only)                */
//   /* ------------------------------------------------------------------ */
...
//       // Only auto-advance for images or text-only. Videos auto-advance via `onEnded`.
...
//                         target="_blank"
//                         rel="noreferrer"
//                         className="bg-blue-600 text-white px-3 py-1 rounded inline-block mb-2 text-sm"
//                       >
...
//                         <span
//                           key={e}
//                           className="bg-white px-2 py-0.5 rounded-full text-sm shadow"
//                         >
//                           {e} {c}
//                         </span>
//                       ))}
...
//                         }
//                         className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 bg-gray-200 text-gray-800 rounded-full w-7 h-7 flex items-center justify-center"
//                       >
//                         More
//                       </button>
//                     )}
...
//   return (
//     <div className="fixed inset-0 bg-black bg-opacity-95 z-[100] flex flex-col items-center justify-center">
//       {/* ... (Status Viewer code is correct, no changes) ... */ }
//        <button onClick={onClose} className="absolute top-4 right-4 z-50">
//         <X className="w-10 h-10 text-white" />
//       </button>
//       <div className="absolute top-4 left-4 flex items-center gap-3 z-50">
...
//   const handleSeek = (e) => {
//     // Get bounds of the progress bar
...
//       socket.current.off("ice-candidate");
//     };
//   }, [me, receiver, call]); // 'me' is a dependency now
